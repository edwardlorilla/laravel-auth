<template>
    <div>
        <v-btn @click="$router.push({path: '/contributions/create'})" color="primary" dark class="mb-2">New Contribution</v-btn>
        <v-card v-if="getUserContribution">
            <v-card-title>
                <span class="headline">Contribution</span>
                <v-container>
                    <v-layout row wrap>
                        <v-flex xs6>
                    ...
                    >
                        <template slot="items" slot-scope="props">
                            ...
                            <td >
                                <v-btn  :disabled="isDisabled" icon class="mx-0"
                                       @click="$router.push({path: '/contribution/edit/' + props.item.id, params: { id: props.item.id }})">
                                    <v-icon color="teal">edit</v-icon>
                                </v-btn>
                                <v-btn :disabled="isDisabled" icon class="mx-0" @click="deleteItem(props.item.id)">
        ...
        data(){
            return {
                ...
                headers: [
                    ...
                    {text: 'Action', value: 'created_at', sortable: false}
                ],
            }
        },
        computed:{
            pluckData() {
                var a = this,
                    b = a.$root.$data.user.contributions,
            ...
            getUserContribution(){
                var vm = this
                return _.filter(vm.$root.$data.user.contributions, ['user_id', vm.$root.$data.user_id])
            }
        },
        methods:{
            deleteItem(id){
                ...
                del('/api/contributions/' + id).then(function () {
                    var index = _.findIndex(vm.$root.$data.user.contributions, ['id', id]);
                    vm.$delete(vm.$root.$data.user.contributions, index)
                    vm.isDisabled = false
